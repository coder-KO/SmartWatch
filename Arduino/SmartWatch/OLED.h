...
class OLED {
  private:
    /**
       Avoid calling externally, use public functions.
    */
  ...
  public:
    ...
    */
    ...
    static void writeByte(byte b, int scaleFactor, int row, int column);
...
static void OLED::writeByte(byte b, int scaleFactor, int row, int column) {
  Wire.beginTransmission(OLED_I2C_ADDRESS);
  Wire.write(OLED_CONTROL_BYTE_DATA_STREAM);
  uint8_t* scaledBytes = OLED::scale(b, scaleFactor);
  // Working with verticle addressing mode
  for (int y = 0; y < scaleFactor; y++) {
    for (int x = scaleFactor - 1; x >= 0; x--) {
      Wire.write(scaledBytes[x]);
